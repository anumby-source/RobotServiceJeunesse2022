...
// A REICHART allume une led pour optique
...
uint32_t Wheel(byte WheelPos) {
  ...
  if(WheelPos < 128) {
    return pixels.Color(WheelPos * 2,255 - WheelPos * 2,0 );
  }
    WheelPos -= 128;
    return pixels.Color(255 - WheelPos * 2,  0,WheelPos * 2 );
};
void neo(int color){ 
  if (color <0 ) color=0;
...
#define NB_COLOR 4 // nombre de couleurs pour la distance
void neo_distance(int color){ 
  if (color <0 ) color=0;
